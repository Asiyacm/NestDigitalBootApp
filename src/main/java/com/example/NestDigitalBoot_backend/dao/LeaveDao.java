...
public interface LeaveDao extends CrudRepository<Leave,Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE `leave_table` SET `status`= 1 WHERE `emp_id`=:emp_id",nativeQuery = true)
    void AcceptLeave(@Param("emp_id") Integer emp_id);

    @Query(value = "SELECT `id`, `apply_date`, `emp_id`, `from_date`, `remarks`, `status`, `to_date`, `type` FROM `leave_table` WHERE `emp_id`=:emp_id",nativeQuery = true)
